...
if __name__ == "__main__":
    ...
    if st.session_state.response_text and st.session_state.response_prompt is None:
        response_prompt = client.models.generate_content(
            model="gemini-2.0-flash",
            contents=st.session_state.response_text + """(For each sentence in the following story that ends with a period (full stop), generate a single, distinct image prompt. The total number of prompts must exactly match the number of sentences ending with a period. Each prompt should vividly and realistically (avoiding anime or cartoonish styles) depict the specific moment described in its corresponding sentence. Write each prompt as a separate paragraph, without any labels or headings.)""",
        )
    ...
    if st.button("Generate Images"):
        ...
        st.session_state.image_files = []  # Reset stored images
                    ...
                    if part.text:
                        ...
                        if "image" in part.inline_data.mime_type:  # Ensure it's an image
                            try:
                                ...
                                # Save image
                                images_save = os.path.join(images_dir, f"image_{idx + 1}.png")
                                image.save(images_save)
                                # Store image path in session state
                                st.session_state.image_files.append(images_save)
                                # Display image in Streamlit
                                # st.image(images_save, caption=f"Image {idx + 1}")
